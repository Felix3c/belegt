...
**Verified (je Anbieter, optional):** `"verified": { "datum": "JJJJ-MM-TT", "rolle": "Datenschutzbeauftragter", "anmerkung": "…" }`. Wird nur gesetzt, wenn (1) AVV, Subprozessorenliste und Trainings-Opt-out durch vom Anbieter benannte Primärdokumente auf `belegt` stehen, (2) die Zero-Data-Retention-Angabe vom Anbieter bestätigt ist — belegt oder ausdrücklich „keine Zusage“, dann als `anmerkung` mit Datum — und (3) eine benannte Rolle das Profil gegengelesen hat. Die Rolle wird veröffentlicht, kein Name. Regeln und Ablauf für Anbieter: Seite `/fuer-anbieter/`, Antwort-Vorlagen in `outreach/anbieter-antwort-vorlage.md`. Kein Logo, keine Gegenleistung, keine Änderung der Reihenfolge.
